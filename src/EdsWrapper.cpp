...
namespace Eds {
	void Retain(EdsBaseRef inRef){EdsError err = EdsRetain(inRef); if(err != EDS_ERR_OK) {throw Exception(err);}}
	void Release(EdsBaseRef inRef){EdsError err = EdsRelease(inRef); if(err != EDS_ERR_OK) {throw Exception(err);}}
